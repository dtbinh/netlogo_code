...
to setup-food  ;; patch procedure
  ...
  if (distancexy (0.6 * max-pxcor) 0) < 9
...
to go  ;; forever button
  ...
  [ if who >= ticks [ stop ] ;; delay initial departure
    ifelse color = red
    [ ;; not carrying food?
      set hunger hunger + 1 ;;no food, must be hungrier
      ifelse hunger > famine-threshold
      [die] ;; too hungry to go on, die - improve model by slowing down ant as it gets hungrier
      [look-for-food]  ;;not dead look for food
    ]
    [ return-to-nest ]       ;; carrying food? take it back to nest
    ;; if we really wanted to improve the model,
    ;; a very hungy ant would drop the food and eat it before getting back to the nest

    wiggle ;; introduce 'randomness' into the system
    fd 1 ]
...
to return-to-nest  ;; turtle procedure
  ifelse nest?
  [
    ;; drop food, eat, and head out again
    set color red
    set hunger 0
    rt 180 ]
...
"food-in-pile4" 1.0 0 -1184463 true "" "plotxy ticks sum [food] of patches with [pcolor = yellow]"
...
"" ""
PENS
"all" 1.0 0 -16777216 true "" "plotxy ticks count turtles"
"Food" 1.0 0 -955883 true "" "plotxy ticks count turtles with [color != red]"
"Searching" 1.0 0 -2674135 true "" "plotxy ticks count turtles with [color = red]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "ifelse any? turtles  [plot mean [hunger] of turtles ] [plot 0]"
